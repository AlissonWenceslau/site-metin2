...
function validatePassword($password)
{
    ...
    if (strlen($password) < $minCaracteres) {
        $errors[] = "Senha conta: A senha deve ter pelo menos $minCaracteres caracteres.";
    }

    if (!preg_match('/[A-Z]/', $password)) {
        $errors[] = "Senha conta: A senha deve conter pelo menos uma letra maiúscula.";
    }

    if (!preg_match('/[a-z]/', $password)) {
        $errors[] = "Senha conta: A senha deve conter pelo menos uma letra minúscula.";
    }

    if (!preg_match('/[0-9]/', $password)) {
        $errors[] = "Senha conta: A senha deve conter pelo menos um número.";
    }

    if (!preg_match('/[\W_]/', $password)) {
        $errors[] = "Senha conta: A senha deve conter pelo menos um caractere especial.";
    }
